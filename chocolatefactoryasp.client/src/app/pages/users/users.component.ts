...
export class UsersComponent implements OnInit {
  ...
  apiUrl = 'https://chocolatefactoryaspserver20250118211324.azurewebsites.net/api/User';
  ...
  registerUser(): void {
    if (this.userForm.valid) {
      ...
      this.http.post(`https://chocolatefactoryaspserver20250118211324.azurewebsites.net/api/Auth/register`, newUser, { headers: this.headers }).subscribe({
        next: () => {
  ...
  activateUser(user: any) {
    console.log(user);
    this.http.patch<any>(`https://chocolatefactoryaspserver20250118211324.azurewebsites.net/api/User/setActive/${user.userId}`, null, { headers: this.headers }).subscribe({
      next: (response) => {
